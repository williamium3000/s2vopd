...
MCQ_BENCHMARKS = [
    ...
    "mmstar",
    "cv-bench",
    # mm-eval/* HF benchmarks (all letter-option MCQ)
    "realworldqa",
    "logicvista",
    "mmbench",
    "mmbench-v11",
...
def pope_extract(text):
    ...
    if t.lower().startswith("answer"):
        # slice instead of case-sensitive split: "Answer: yes" used to crash here
...
# Benchmarks whose responses are long reasoning chains ending in a final answer.
# ONLY these get the long-CoT extraction below; every other benchmark keeps the
# legacy extraction behavior byte-identical, regardless of response length.
LONG_COT_BENCHMARKS = {"mathvista", "mathverse", "mathvision", "mathvision-op", "wemath"}


def extract_answer(model_answer_raw, long_cot=False):
    ...
    # Legacy benchmarks, or short responses: byte-identical legacy behavior.
    if not long_cot or len(text) <= 6000:
        return text
    # Long CoT (math chains) overflow the judge's context and bury the conclusion.
    # Extract it: last \boxed{...}, else text after the last "answer is/answer:",
    # else the tail of the response.
    ...
    ans = list(re.finditer(r"(?:final answer|answer)\s*(?:is|:)", text, re.IGNORECASE))
    if ans:
        seg = text[ans[-1].end() : ans[-1].end() + 400].lstrip(":").strip()
        if seg:
            return seg
    return text[-600:]
...
def main():
    ...
    for i, item in enumerate(tqdm(data_list, desc="Rule-based grading")):
        ...
        extracted_answer = extract_answer(model_answer_raw, long_cot=args.benchmark in LONG_COT_BENCHMARKS)
        gt = item["response"]
