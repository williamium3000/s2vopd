...
declare -A BENCHMARK_JSON_MAP=(
  ...
  [mathvision-op]="mathvision_op.json"
  [wemath]="wemath.json"
...
run_single_benchmark() {
  ...
  [[ -n "${TEMPERATURE}" ]] && INFER_ARGS+=(--temperature "${TEMPERATURE}")
  [[ -n "${TOP_P}" ]] && INFER_ARGS+=(--top_p "${TOP_P}")
  [[ -n "${TOP_K}" ]] && INFER_ARGS+=(--top_k "${TOP_K}")
  [[ -n "${PRESENCE_PENALTY}" ]] && INFER_ARGS+=(--presence_penalty "${PRESENCE_PENALTY}")
