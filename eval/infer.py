...
def main():
    ...
    # Sampling overrides. Defaults preserve the legacy greedy protocol exactly;
    # set them only when matching a model's official generation config.
    ...
    def run_one(item):
        ...
        # Send ALL images in order (multi-image benchmarks e.g. BLINK); falls back
        # to the single image[0] for the existing single-image benchmarks.
        img_paths = item.get("images") or [pick_image_path(item)]
        content = [{"type": "image_url", "image_url": {"url": image_to_data_uri(p, benchmark)}}
                   for p in img_paths]
        ...
        for attempt in range(1, args.max_retries + 1):
            try:
                ...
                if args.enable_thinking is not None:
                    extra_body["chat_template_kwargs"] = {"enable_thinking": args.enable_thinking == "True"}
