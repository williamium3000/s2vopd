...
def calc_hallusion(judge_json, benchmark, benchmark_json=None):
    """HallusionBench: aAcc (per-question) + qAcc (per question-set) + fAcc (per figure)."""
    with open(judge_json, "r", encoding="utf-8") as f:
        data = json.load(f)
    n = len(data)
    c = sum(1 for x in data if is_correct(x))
    print(f"{benchmark} aAcc: {c}/{n} = {100 * c / n:.2f}%")

    def group_acc(key_fn):
        groups = {}
        for x in data:
            groups.setdefault(key_fn(x), []).append(is_correct(x))
        ok = sum(1 for v in groups.values() if all(v))
        return ok, len(groups)

    # group keys must include the category/subcategory (stored in `task`):
    # set_id values repeat across categories and would otherwise collide
    qc, qn = group_acc(lambda x: (x.get("task"), x.get("set_id"), str(x.get("question_id", "")).rsplit("_", 1)[-1]))
    fc, fn = group_acc(lambda x: (x.get("task"), x.get("set_id"), x.get("figure_id")))
    print(f"{benchmark} qAcc: {qc}/{qn} = {100 * qc / max(qn,1):.2f}%")
    print(f"{benchmark} fAcc: {fc}/{fn} = {100 * fc / max(fn,1):.2f}%")


def calc_blink(judge_json, benchmark):
    """BLINK: 14 sub-tasks (category field). Report per-sub-task, plus pooled (micro,
    the metric simple-mmeval reports) and macro (mean of sub-tasks)."""
    with open(judge_json, "r", encoding="utf-8") as f:
        data = json.load(f)
    sub = defaultdict(lambda: {"correct": 0, "total": 0})
    for item in data:
        ...
        if is_correct(item):
            sub[cat]["correct"] += 1
    accs = []
    for cat in sorted(sub):
        s = sub[cat]
        a = 100.0 * s["correct"] / s["total"] if s["total"] else 0.0
        accs.append(a)
        print(f"  {cat}: {a:.2f}% ({s['correct']}/{s['total']})")
    c = sum(s["correct"] for s in sub.values())
    n = sum(s["total"] for s in sub.values())
    macro = sum(accs) / len(accs) if accs else 0.0
    print(f"{benchmark} Acc: {c}/{n} = {100 * c / n:.2f}%  (micro/pooled <- reported)")
    print(f"{benchmark} macro-avg ({len(accs)} sub-tasks): {macro:.2f}%")
...
def main():
    ...
    if args.benchmark == "visualprobe":
        ...
        calc_blink(args.judge_json, args.benchmark)
    elif args.benchmark == "hallusionbench":
        calc_hallusion(args.judge_json, args.benchmark, args.benchmark_json)
